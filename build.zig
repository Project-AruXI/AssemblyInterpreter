...
pub fn build(b: *std.Build) void {
  ...
  const archModule = b.addModule("arch", .{
    .root_source_file = b.path("common/defs/arch.zig"),
    .target = target
  });

  const opcodesModule = b.addModule("opcodes", .{
    .root_source_file = b.path("common/defs/opcodes.zig"),
    .target = target
  });
  ...
  const engineNative = b.addLibrary(.{
    ...
    .root_module = b.createModule(.{
      ...
      .optimize = optimize,
      .imports = &.{
        .{ .name = "arch", .module = archModule },
        .{ .name = "opcodes", .module = opcodesModule }
      },
    }),
    .use_llvm = true
  });
